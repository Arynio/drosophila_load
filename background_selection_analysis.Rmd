...
#However, the files in BOT_EXPAN are not filtered by A8. To do so:
cd /share/rdata2/dani_k/seleccion_fondo/plink/BOT_EXPAN
##For autosomes:
#awk '{print $1, $4}' /share/rdata2/dani_k/seleccion_fondo/plink/BOT/A8.map | sort > /share/rdata2/dani_k/seleccion_fondo/plink/BOT/array_snp_A8_sorted.txt
awk '{print $1, $4}' A60.map | sort > array_snp_A60_sorted.txt
comm -12 /share/rdata2/dani_k/seleccion_fondo/plink/BOT/array_snp_A8_sorted.txt array_snp_A60_sorted.txt > intersecting_snps_A60_A8.txt
awk '{print $1, $2, $2, NR}' intersecting_snps_A60_A8.txt > intersecting_ranges_A60_A8.txt
/share/rdata2/dani_k/seleccion_fondo/plink/plink --file A60 --extract range intersecting_ranges_A60_A8.txt --make-bed --recode --out A60-A8
##For Xchr:
!!!IT DOESNT WORK!
awk '{print $1, $4}' /share/rdata2/dani_k/seleccion_fondo/plink/BOT/A8X.map | sort > /share/rdata2/dani_k/seleccion_fondo/plink/BOT/array_snp_A8X_sorted.txt
awk '{print $1, $4}' A60X.map | sort > array_snp_A60X_sorted.txt
comm -12 /share/rdata2/dani_k/seleccion_fondo/plink/BOT/array_snp_A8X_sorted.txt array_snp_A60X_sorted.txt > intersecting_snps_A60X_A8.txt
awk '{print $1, $2, $2, NR}' intersecting_snps_A60X_A8.txt > intersecting_ranges_A60X_A8.txt
/share/rdata2/dani_k/seleccion_fondo/plink/plink --file A60X --extract range intersecting_ranges_A60X_A8.txt --make-bed --recode --out A60X-A8
...
##TFM:
###Prepare filtering file:
```{bash}

module load gcc/7.2.0 
module add gcc/7.2.0 

mkdir -p /share/rdata2/dani_k/seleccion_fondo/TFM/
cd /share/rdata2/dani_k/seleccion_fondo/TFM/

#Combine all desired exclusion tracks (Humberto chose them) into a single one:
cat /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Pb-B-ALL_merged_snps_excised_autosomic.nr_annovar.dm6nr_multianno.missense_and_LoF.bed /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Pb-B-ALL_merged_snps_excised_autosomic.nr_annovar.dm6nr_multianno.UTR.bed <(cut -f-3 /share/rdata2/dani_k/seleccion_fondo/tracks/phastCons_exclude_regions.bed) | sortBed | mergeBed | sed -e 's/^2L/1/g' -e 's/^2R/2/g' -e 's/^3L/3/g' -e 's/^3R/4/g' -e 's/^X/5/g' > exclude_phastCons_UTR_missense_LoF.bed

#Tranform it into a 1-based .txt file that includes row numbers, which is what plink needs:
sort exclude_phastCons_UTR_missense_LoF.bed | awk '{print $1, $2+1, $3, NR}' > exclude_phastCons_UTR_missense_LoF.txt

```

###Filter the plink files:
####Populations in the experiment by Irene:
```{bash}
...
cd /share/rdata2/dani_k/seleccion_fondo/TFM/
scp -p /share/rdata2/datos_procesados/mapped/* ./ #Humberto put the .ped and .map files in this folder.

#Filter the files:
FILE=$(ls *.ped | cut -d'.' -f1)
for file in ${FILE[@]}
  do
  /share/rdata2/dani_k/seleccion_fondo/plink/plink --file $file --exclude range exclude_phastCons_UTR_missense_LoF.txt --make-bed --recode --out ${file}_filtered
  done

#Fix the genetic distances:
##For autosomes:
MAP_FILES=$(ls *_filtered.map | grep -v 'chrX')
for map in ${MAP_FILES[@]}
  do
  awk '{printf ("%s\t%s\t%s\t%s\n",$1, $2, $3/2, $4)}' $map > ${map/.map/.mod.map} && mv ${map/.map/.mod.map} $map
  done
##For the Xchr:
MAP_FILES=$(ls *_filtered.map | grep 'chrX')
for map in ${MAP_FILES[@]}
  do
  awk '{printf ("%s\t%s\t%s\t%s\n",$1, $2, $3*2/3, $4)}' $map > ${map/.map/.mod.map} && mv ${map/.map/.mod.map} $map
  done

#Copy the files to the destination folder using invitado1:
mkdir -p /home/invitado1/TFG_YOUSSEF_DATOS_REALES
scp -p /share/rdata2/dani_k/seleccion_fondo/TFM/*_filtered.ped /home/invitado1/TFG_YOUSSEF_DATOS_REALES/
scp -p /share/rdata2/dani_k/seleccion_fondo/TFM/*_filtered.map /home/invitado1/TFG_YOUSSEF_DATOS_REALES/

#Rename them using ivitado1:
cd /home/invitado1/TFG_YOUSSEF_DATOS_REALES
FILES=$(ls *_chrX_filtered*)
for f in ${FILES[@]}; do mv $f ${f/_filtered/}; done
FILES=$(ls *_filtered*)
for f in ${FILES[@]}; do mv $f ${f/_filtered/_autosomes}; done
...
#If genetic distances are to be added to the map files without losing sites (those not present in Pb-B-ALL, which is where genetic distances have been calculated), perhaps another option would be to calculate the average between the preceeding and the following SNP.
...
##For autosomes:
LINES=$(bcftools query -l /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllinesonly-Allcomb_autohap_genotyped_snps_masked_hf_pass.vcf | cut -c 2-3 | sort -u)
for pop in ${LINES[@]}
  do
  echo "${pop}"
  cd /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/linea_"${pop}"/dani/
  #Filter the files:
  /share/rdata2/dani_k/seleccion_fondo/plink/plink --file L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass --exclude range /share/rdata2/dani_k/seleccion_fondo/TFM/exclude_phastCons_UTR_missense_LoF.txt --make-bed --recode --out L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered
  #Add proper genetic distances (this doesn't work fine because some sites not present in Pb-B_ALL are lost from the map, but not from the ped):
  #bedtools intersect -a <(awk -F"\t" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$4-1,$4,$2,$3)}' L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered.map) -b <(awk -F"\t" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$4-1,$4,$2,$3/2)}' /share/rdata2/dani_k/seleccion_fondo/plink/genetic_map/Pb-B-ALL_merged_snps_final_autosomic.map) -wa -wb | awk -F"\t" '{printf ("%s\t%s\t%s\t%s\n", $1,$4,$10,$3)}' > L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered.map.bis
  #mv L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered.map.bis L"${pop}"-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered.map
  done
##For the Xchr:
LINES=$(bcftools query -l /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllinesonly-Allcomb_autohap_genotyped_snps_masked_hf_pass.vcf | cut -c 2-3 | sort -u)
for pop in ${LINES[@]}
  ...
  cd /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/linea_"${pop}"/dani/
  #Filter the files:
  /share/rdata2/dani_k/seleccion_fondo/plink/plink --file L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass.corrected --exclude range /share/rdata2/dani_k/seleccion_fondo/TFM/exclude_phastCons_UTR_missense_LoF.txt --make-bed --recode --out L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered
  #Add proper genetic distances (this doesn't work fine because some sites not present in Pb-B_ALL are lost from the map, but not from the ped):
  #bedtools intersect -a <(awk -F"\t" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$4-1,$4,$2,$3)}' L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered.map) -b <(awk -F"\t" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$4-1,$4,$2,$3*2/3)}' /share/rdata2/dani_k/seleccion_fondo/plink/genetic_map/Pb-B-ALL_merged_snps_final_X.map) -wa -wb | awk -F"\t" '{printf ("%s\t%s\t%s\t%s\n", $1,$4,$10,$3)}' > L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered.map.bis
  #mv L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered.map.bis L"${pop}"-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered.map
  done

#Copy the files to the destination folder using invitado1:
mkdir -p /home/invitado1/TFG_YOUSSEF_DATOS_REALES
scp -pr /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/linea_*/dani/*_filtered.ped /home/invitado1/TFG_YOUSSEF_DATOS_REALES/
scp -pr /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/linea_*/dani/*_filtered.map /home/invitado1/TFG_YOUSSEF_DATOS_REALES/

#Rename them using ivitado1:
cd /home/invitado1/TFG_YOUSSEF_DATOS_REALES
FILES=$(ls L*autohap*)
for f in ${FILES[@]}; do mv $f ${f/-Allcomb_autohap_genotyped_snps_masked_hf_pass_filtered/_autosomes}; done
FILES=$(ls L*xhap*)
for f in ${FILES[@]}; do mv $f ${f/-Allcomb_xhap_genotyped_snps_masked_hf_pass_filtered/_chrX}; done
