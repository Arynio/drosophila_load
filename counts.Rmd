...
VCF="/share/rdata/ramon.pouso/POOLS/all_gens_0_140/variants/crisp_all_pools_gen0-140.recode_snps.gen0-140_Pb_0_variable_set.orthologs.confident.polarized.annovar.dm6_multianno.custom_complete.vcf"
SUBSET="gen0-140"
echo $SUBSET
...
######New version, extended categories:
```{R, engine='bash'}
...
module add gcc/7.2.0
cd /share/rdata/ramon.pouso/counts/sites_in_0-140_VCF/pools_counts/$SUBSET/
rm counts_pool_${SUBSET}_${REGION}_summary.complete.txt
echo -e "sample\tgeneration\tnon-fourfold_V\tnon-fourfold_D\tfourfold_V\tfourfold_D\ttolerated_V\ttolerated_D\tdeleterious_V\tdeleterious_D\tLoF_V\tLoF_D" > counts_pool_${SUBSET}_${REGION}_summary.complete.txt
POOL_LIST=($(ls -v `find . -maxdepth 1 -name '*_'${SUBSET}'_pool.txt' -print`))
for pool in "${POOL_LIST[@]}"
  do
  echo "${pool}"
  SAMPLE=$(echo "${pool}" | cut -d'/' -f2 | cut -d'_' -f1)
  ...
  NONFOURFOLD_D=$(grep "CUSTOM=non-fourfold;" $p | awk '{print $10}' | paste -sd+ | bc)
  FOURFOLD_V=$(grep "CUSTOM=fourfold;" $p | wc -l)
  FOURFOLD_D=$(grep "CUSTOM=fourfold;" $p | awk '{print $10}' | paste -sd+ | bc)
  MISTOL_V=$(grep "CUSTOM=tolerated;" $p | wc -l)
  MISTOL_D=$(grep "CUSTOM=tolerated;" $p | awk '{print $10}' | paste -sd+ | bc)
  MISDEL_V=$(grep "CUSTOM=deleterious;" $p | wc -l)
  MISDEL_D=$(grep "CUSTOM=deleterious;" $p | awk '{print $10}' | paste -sd+ | bc)
  LOF_V=$(grep "CUSTOM=LoF;" $p | wc -l)
  LOF_D=$(grep "CUSTOM=LoF;" $p | awk '{print $10}' | paste -sd+ | bc)
  echo -e "$SAMPLE\t$GEN\t$NONFOURFOLD_V\t$NONFOURFOLD_D\t$FOURFOLD_V\t$FOURFOLD_D\t$MISTOL_V\t$MISTOL_D\t$MISDEL_V\t$MISDEL_D\t$LOF_V\t$LOF_D" >> counts_pool_${SUBSET}_${REGION}_summary.complete.txt
  done
...
module add gcc/7.2.0
cd /share/rdata/ramon.pouso/counts/sites_in_0-140_VCF/pools_counts/$SUBSET/key_categories
rm counts_pool_${SUBSET}_${REGION}_summary.validated_subset.txt
echo -e "sample\tgeneration\tfourfold_V\tfourfold_D\ttolerated_V\ttolerated_D\tdeleterious_V\tdeleterious_D\tLoF_V\tLoF_D" > counts_pool_${SUBSET}_${REGION}_summary.validated_subset.txt
POOL_LIST=($(ls -v `find . -name '*_'${SUBSET}'_pool.txt' -print`))
for pool in "${POOL_LIST[@]}"
  do
  echo "${pool}"
  ...
  LOF_V=$(grep "CUSTOM=LoF;" $p | wc -l)
  LOF_D=$(grep "CUSTOM=LoF;" $p | awk '{print $11}' | paste -sd+ | bc)
  echo -e "$SAMPLE\t$GEN\t$FOURFOLD_V\t$FOURFOLD_D\t$MISTOL_V\t$MISTOL_D\t$MISDEL_V\t$MISDEL_D\t$LOF_V\t$LOF_D" >> counts_pool_${SUBSET}_${REGION}_summary.validated_subset.txt
  done

scp ramon.pouso@rua2.uvigo.es://share/rdata/ramon.pouso/counts/sites_in_0-140_VCF/pools_counts/gen0-140/counts_pool_gen0-140_all_summary.validated_subset.txt /Users/dani/Documents/dani/Post-docs/2022_contrato_Vigo/drosophila_load/Analisis_dani/counts/gen0-140/
...
wd_path <- ("/Users/dani/Documents/dani/Post-docs/2022_contrato_Vigo/drosophila_load/Analisis_dani/counts/sites_in_Pb000/original_dataset/")
SFS_raw_data_gen0_40 <- read_tsv(paste0(wd_path,"SFS_pool_gen0-40_combined_",type,"_summary.txt")) %>% mutate(gen=ifelse(sample=="LBT0",000,ifelse(sample=="c1",005,ifelse(sample=="BT20",020,ifelse(sample=="BT30",030,040)))))
SFS_raw_data_gen140 <- read_tsv(paste0(wd_path,"SFS_pool_gen140_combined_",type,"_summary.txt")) %>% mutate(gen="140" %>% as.double)
