...
#Go to the all_sites_pipeline.Rmd script.
...
#Obsolete, should be applied to the DP_EM_filtered VCFs!!
...
while read -r SCAFFOLD START STOP ANCESTRAL REFERENCE; do
 ...
 done < /share/rdata/ramon.pouso/polarisation/estsfs/output/obsolete/final_snp_set.orthologs.sum_gen0_gen20_gen30_allele_counts.extrapolated_to_100.ancestral_vs_derived_complete.confident_inconsistent.rate6.pval.bed
...
#Now that the AA information has been retrieved, for each dataset combine the autosomic and the X files and then filter the resulting VCF in order to keep only those positions with AA information. The DP_EM_filtered version of the VCFs was already used as the starting point to select the variants in the bed file used here, so it's OK to use the unfiltered version of the VCF here.
module load bcftools/1.9
module load gcc/7.2.0
module add gcc/7.2.0
cd /share/rdata/ramon.pouso/POOLS/gen0-40/gen0-40/5variants/multirealigned/

#First generate list of common names for both VCF files (remove the autosomic/x part):
bcftools query -l CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.vcf | rev | cut -d'_' -f2- | rev > sample_names.list

#Then for the autosomic file, change the names, compress the file and index it:
bcftools reheader --samples sample_names.list CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.vcf > CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.fixed_names.vcf
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/bgzip -c CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.fixed_names.vcf > CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.fixed_names.vcf.gz
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/tabix -p vcf CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.fixed_names.vcf.gz

#Same for the Xchr file:
bcftools reheader --samples sample_names.list CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.vcf > CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.fixed_names.vcf
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/bgzip -c CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.fixed_names.vcf > CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.fixed_names.vcf.gz
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/tabix -p vcf CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.fixed_names.vcf.gz

#Finally, concatenate both files and keep only the previously selected orthologous high confidence SNPs.
bcftools concat CRISP_autosomic/masked/crisp_multi_all_masked.recode_snps.fixed_names.vcf.gz CRISP_x/masked/crisp_multi_all_x_masked.recode_snps.fixed_names.vcf.gz | bedtools intersect -a stdin -b /share/rdata/ramon.pouso/polarisation/estsfs/output/final_snp_set.orthologs.sum_gen0_gen20_gen30_allele_counts.extrapolated_to_100.ancestral_vs_derived_complete.confident.rate6.pval.bed -header > crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.vcf

#And edit some problematic spaces in the header (otherwise vcftools won't be able to deal with them later on):
sed -i 's/Description=" >/Description=">/g' crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.vcf
...
#First generate list of common names for both VCF files (remove the autosomic/x part):
bcftools query -l crisp_all_gen140_autosomic_masked.recode_snps.vcf | rev | cut -d'_' -f2- | rev > sample_names.list

#Then for the autosomic file, change the names, compress the file and index it:
bcftools reheader --samples sample_names.list crisp_all_gen140_autosomic_masked.recode_snps.vcf > crisp_all_gen140_autosomic_masked.recode_snps.fixed_names.vcf
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/bgzip -c crisp_all_gen140_autosomic_masked.recode_snps.fixed_names.vcf > crisp_all_gen140_autosomic_masked.recode_snps.fixed_names.vcf.gz
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/tabix -p vcf crisp_all_gen140_autosomic_masked.recode_snps.fixed_names.vcf.gz

#Same for the Xchr file:
bcftools reheader --samples sample_names.list crisp_all_gen140_x_masked.recode_snps.vcf > crisp_all_gen140_x_masked.recode_snps.fixed_names.vcf
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/bgzip -c crisp_all_gen140_x_masked.recode_snps.fixed_names.vcf > crisp_all_gen140_x_masked.recode_snps.fixed_names.vcf.gz
/DATA/APPS/freebayes/25.03.19/SeqLib/htslib/tabix -p vcf crisp_all_gen140_x_masked.recode_snps.fixed_names.vcf.gz

#Finally, concatenate both files and keep only the previously selected orthologous high confidence SNPs.
bcftools concat crisp_all_gen140_autosomic_masked.recode_snps.fixed_names.vcf.gz crisp_all_gen140_x_masked.recode_snps.fixed_names.vcf.gz | bedtools intersect -a stdin -b /share/rdata/ramon.pouso/polarisation/estsfs/output/final_snp_set.orthologs.sum_gen0_gen20_gen30_allele_counts.extrapolated_to_100.ancestral_vs_derived_complete.confident.rate6.pval.bed -header > crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.vcf
...
#Go to script: individuals_VCF_filtering.Rmd
#The final datasets are the following:
##Autosomes:
/share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_hap_genotyped_snps_masked_hf_pass.final_snp_set.vcf
##Xchr:
/share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_xhap_genotyped_snps_masked_hf_pass.final_snp_set.vcf
...
#I launched fill-aa.sh (locally, not with qsub due to problems with PERL5LIB) for the following VCF files:

##Pools gens0-40 (autosomic + Xchr):
/share/rdata/ramon.pouso/Scripts/fill-aa.sh /share/rdata/ramon.pouso/POOLS/gen0-40/gen0-40/5variants/multirealigned/crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.vcf
##Pools gen140 (autosomic + Xchr):
/share/rdata/ramon.pouso/Scripts/fill-aa.sh /share/rdata/ramon.pouso/POOLS/gen140/gen140/6variants/masked/crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.vcf
##Individuals, all lines (autosomic):
/share/rdata/ramon.pouso/Scripts/fill-aa.sh /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_hap_genotyped_snps_masked_hf_pass.final_snp_set.vcf
##Individuals, all lines (Xchr):
/share/rdata/ramon.pouso/Scripts/fill-aa.sh /share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_xhap_genotyped_snps_masked_hf_pass.final_snp_set.vcf

```

##Polarize the AA-filled VCF.
###POOLS.
```{bash}

#Polarize the AA-filled VCF based on the new INFO/AA column. Alleles will be switched whenever the ancestral allele matches the alternative one, and AD and AF columns will be properly recoded. Not the rest!!!

module load bcftools/1.9 

FILE=/share/rdata/ramon.pouso/POOLS/gen140/gen140/6variants/masked/crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.aafilled.vcf
#/share/rdata/ramon.pouso/POOLS/gen0-40/gen0-40/5variants/multirealigned/crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.aafilled.vcf #/share/rdata/ramon.pouso/POOLS/gen140/gen140/6variants/masked/crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.aafilled.vcf
...
#Sanity checks:
##No rows were dropped in the gens0-40 polarized file.
##The gen140 polarized file has 853089 rows instead of 853373 because there were 284 SNPs with AA!=REF & AA!=ALT (a third allele), which were dropped. Check the number of dropped SNPs using: bedtools subtract -a crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.aafilled.vcf -b crisp_multi_all_masked.recode_snps.final_snp_set.orthologs.confident.polarized.vcf | wc -l
...
FILE=/share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_hap_genotyped_snps_masked_hf_pass.final_snp_set.aafilled.vcf #/share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_hap_genotyped_snps_masked_hf_pass.final_snp_set.aafilled.vcf #/share/rdata/ramon.pouso/INDIVIDUOS/gen40/gen40/6variants/Alllines-Allcomb_xhap_genotyped_snps_masked_hf_pass.final_snp_set.aafilled.vcf

java -jar /DATA/APPS/jvarkit/dist/vcffilterjdk.jar -e 'if(variant.getNAlleles()!=2 || !variant.hasAttribute("AA")) return true; 
...
return vb.make();' -o ${FILE/.aafilled.vcf/.polarized.vcf} $FILE
